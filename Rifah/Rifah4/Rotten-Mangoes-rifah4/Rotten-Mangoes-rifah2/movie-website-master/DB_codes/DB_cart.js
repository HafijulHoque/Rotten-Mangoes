...
async function getTotalPrice(cartid){
    ...
    console.log(cartid);
    let sql = `
    SELECT * FROM "C##MOVIEDATABASE"."Merchandise"
    WHERE "Product_id" IN (
        SELECT "Product_id" FROM "C##MOVIEDATABASE"."Merch_order"
...
async function getCartId(username){
    let sql = `
    SELECT "Cart_id" FROM "C##MOVIEDATABASE"."Cart"
...
async function changeBool1(product_id, username){
    let sql = `
    UPDATE "C##MOVIEDATABASE"."Merch_order"
    SET "Bool" = 1
